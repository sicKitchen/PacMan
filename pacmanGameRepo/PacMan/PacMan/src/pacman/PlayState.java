...
class PlayState extends BasicGameState {
	...
	public void update(GameContainer container, StateBasedGame game,
		...
		if (pacdots_remaining <= 0){
			pg.enterState(pacmanGame.LVL2STATE);	
		}
		...
		for (Iterator<PowerPill> i = pg.powerpill_array.iterator(); i.hasNext();) {
			...
			if(tmp_Pill.grid_x == pg.pacman.grid_x && tmp_Pill.grid_y == pg.pacman.grid_y && tmp_Pill.Visable == true){
				...
				Scatter_Countdown = 8000;  // <------------ Change how long power Scatter State is active here
	...
	@Override
	public int getID() {
		return pacmanGame.PLAYINGSTATE;
	}
